import sys
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QLabel, QListWidget, QPushButton, QLineEdit, QHBoxLayout, QMessageBox
...
class TransactionWidget(QWidget):
    ...
    def send_transaction(self):
        ...
        if self.selected_wallet_address is None:
            QMessageBox.warning(self, 'No Wallet Selected', 'Please select a wallet before sending a transaction.', QMessageBox.Ok)
            return
        ...
        if not amount_text:
            QMessageBox.warning(self, 'Invalid Amount', 'Please enter a valid amount before sending a transaction.', QMessageBox.Ok)
            return
        ...
        try:
            ...
            QMessageBox.warning(self, 'Invalid Amount', 'Please enter a valid numeric amount.', QMessageBox.Ok)
            return

        # Get the selected wallet
        selected_wallet = self.wallet_manager.get_wallet_by_address(self.selected_wallet_address)
        ...
        if selected_wallet is not None:
            ...
            QMessageBox.information(self, 'Transaction Sent', f'Transaction sent!\nTransaction ID: {transaction_id}', QMessageBox.Ok)
