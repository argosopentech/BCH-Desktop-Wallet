...
import sys
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QListWidget, QLabel, QPushButton, QHBoxLayout, QFileDialog
...
class PrivateKeyViewer(QWidget):
    ...
    def init_ui(self):
        ...
        self.address_list_widget.addItems([wallet['address'] for wallet in self.local_wallet_manager.get_wallets()])
    ...
    def show_private_key(self, item):
        ...
        if wallet:
            ...
            self.private_key_label.setText(f"Private Key for {selected_address}: {private_key}")
        else:
            self.private_key_label.setText("No private key found for the selected address.")

        self.path_label.setText(f"Private keys stored in: {self.local_wallet_manager.wallet_file_path}")
